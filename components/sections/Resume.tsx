...
export function Resume() {
    ...
    const courses = experienceData.filter((exp) => exp.type === "course");
    const interests = experienceData.filter((exp) => exp.type === "interest");

    return (
        <Section
            id="resume"
            title="Resume"
            subtitle="Education, responsibility, achievements, courses, and interests"
        >
            ...
            {/* Courses */}
            <div className="mb-12">
                <h3 className="text-2xl font-semibold text-accent mb-6">Key Courses</h3>
                <div className="space-y-4">
                    {courses.map((course, idx) => (
                        ...
                        >
                            <div className="flex-1">
                                <h4 className="text-lg font-semibold text-text-primary">{course.role}</h4>
                                <p className="text-accent font-medium mb-3">{course.company}</p>
                                <ul className="space-y-2">
                            ...
                            <span className="text-sm text-text-secondary font-mono bg-background px-3 py-1 rounded-full border border-border w-fit">{course.period}</span>
                        </div>
                    ))}
                </div>
            </div>
            ...
            {/* Interests */}
            <div className="mb-12">
                <h3 className="text-2xl font-semibold text-accent mb-6">Interests</h3>
                <div className="flex flex-wrap gap-3">
                    {interests.flatMap((interest) => interest.description).map((item) => (
                        <span
                            key={item}
                            className="px-4 py-2 bg-surface border border-border rounded-lg text-text-secondary"
                        >
                            {item}
                        </span>
                    ))}
            ...
            <div className="flex justify-center mt-12 px-4 sm:px-0">
                <Link href="/resume/Mahesh_Tadepalli_Resume.pdf" target="_blank" rel="noopener noreferrer" className="w-full sm:w-auto">
